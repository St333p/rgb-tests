...
#[test]
fn no_double_spend() {
    use amplify::confinement::{LargeOrdSet, NonEmptyOrdSet};
    ...
    bundles.remove(&new_bundle); // otherwise it doesn't get replaced
    bundles.insert(new_bundle);
    consignment.bundles = LargeOrdSet::from_checked(bundles);
    // remove corresponding terminal
    consignment.terminals.remove(&bundle_id).unwrap(); // remove to be sure it's replaced on insert
    consignment
        .terminals
